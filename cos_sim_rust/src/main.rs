...
fn threading (original_phrases: Vec<String>,  phrases_to_compare: Vec<String>, thread_count: usize)-> (Vec<Vec<usize>>, Vec<Vec<usize>>) {
...
fn main() {
	let phrases_1 : Vec<String> = vec!["this is a very long sample phrase".to_string(), "phrase".to_string()];
	...
	let (vec_1, vec_2) = threading(phrases_1, phrases_2, 5);
	let sim = cosine_sim(vec_1, vec_2);
	println!{"the similarity is {}", sim}
}
